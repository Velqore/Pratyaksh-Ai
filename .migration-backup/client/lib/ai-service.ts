...
class AIForensicService {
  ...
  async analyzeWithAI(request: AIAnalysisRequest): Promise<any> {
    try {
      console.log(
        `🤖 AI Analysis started for ${request.type}: ${request.file.name}`,
      );

      // Generate deterministic seed from file content for consistent results
      const fileSeed = await generateSeedFromFile(request.file);
      const random = new SeededRandom(fileSeed);

      // Get previous learning data for this analysis type
      const previousLearning = this.learningDatabase.get(request.type) || [];

      // Perform base analysis with fallback
      let baseAnalysis;
      try {
        baseAnalysis = await evidenceApi.analyzeEvidence(
          request.file,
          request.type,
          `AI Analysis - ${request.file.name}`,
          `Advanced AI-powered ${request.type} analysis with machine learning enhancement`,
        );
      ...
      try {
        switch (request.type) {
          case "cyber":
            enhancedAnalysis = await this.enhanceCyberAnalysis(
              request.file,
              baseAnalysis,
            );
            break;
          case "fingerprint":
            ...
            break;
          case "document":
            enhancedAnalysis = await this.enhanceDocumentAnalysis(
              request.file,
              baseAnalysis,
              previousLearning,
            );
        ...
        console.error(
          "❌ Enhancement failed, using base analysis:",
          enhancementError,
        );
        enhancedAnalysis = baseAnalysis;
      }

      // Learn from this analysis (with error handling)
      try {
      ...
      return {
        ...
        threatIntelligence: this.getThreatContext(request.file.name),
        processingInfo: {
      ...
      // Return a minimal but functional result
      return this.generateMinimalFallbackResult(request, error);
    }
  }
